...
public class DanhMucChiFragment extends Fragment {
    private View view;
    private RecyclerView rvDanhMucChi;
    private AppViewModel appViewModel;
    private DanhMucAdapter danhMucAdapter;
    private LiveData<List<DanhMuc>> listDanhMuc;
    private DanhMuc danhMucChon;
    private TextView txtTongSoDanhMuc;
    private int soDanhMucChi;
    @Nullable
    @org.jetbrains.annotations.Nullable
    @Override
    public View onCreateView(@NonNull @NotNull LayoutInflater inflater, @Nullable @org.jetbrains.annotations.Nullable ViewGroup container, @Nullable @org.jetbrains.annotations.Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_danhmucchi,container,false);

        appViewModel = new ViewModelProvider(this).get(AppViewModel.class);
        rvDanhMucChi = view.findViewById(R.id.rvDanhMucChi);
        rvDanhMucChi.setLayoutManager(new GridLayoutManager(getContext(), 4));
        rvDanhMucChi.setHasFixedSize(true);
        danhMucAdapter = new DanhMucAdapter(getContext());
        rvDanhMucChi.setAdapter(danhMucAdapter);
        listDanhMuc = appViewModel.tatCaDanhMucChi();
        listDanhMuc.observe(getActivity(), new Observer<List<DanhMuc>>() {
            ...
            public void onChanged(List<DanhMuc> danhMucList) {
                ...
                soDanhMucChi = danhMucList.size();
                txtTongSoDanhMuc.setText("Tổng số: " + soDanhMucChi);
            }
        });
